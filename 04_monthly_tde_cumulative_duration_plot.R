...
df_T3_ALL_cum_summary <- df_T3_ALL |>
  ...
  summarise(
    med = median(Cumulative_duration_hours_monthly, na.rm = TRUE),
    Q1 = quantile(Cumulative_duration_hours_monthly, 0.25, na.rm = TRUE),
    Q3 = quantile(Cumulative_duration_hours_monthly, 0.75, na.rm = TRUE),
    .groups = "drop"
  )

pb <- ggplot(
  df_T3_ALL,
  aes(x = Months_From_Earliest, y = Cumulative_duration_hours_monthly, color = Class)
) +
  geom_pointrange(
    data = df_T3_ALL_cum_summary,
  ...
  labs(
    x = "Months",
    y = "Hours",
    title = "B. Cumulative monthly duration (TDE)",
...
ggsave(
  filename = "~/Downloads/figure_2b_monthly_tde_cumulative_duration.jpg",
  plot = pb,
  dpi = 600,
  width = 12,
  height = 8,
