...
df_wide <- spread(df, key = Categories, value = Absolute_duration_hours_weekly)
...
catVars <- c("Class")
file_prefix <- "s_tab_24_abs_weekly_duration_psychological_apps_class3_vs_class4_week_"

dir.create(
  "~/Downloads/supplementary_table_24_output/",
  recursive = TRUE,
  showWarnings = FALSE
)
...
write.csv(
  merged_data,
  file = "~/Downloads/supplementary_table_24_abs_weekly_duration_psychological_apps_class3_vs_class4.csv",
  row.names = FALSE
)
