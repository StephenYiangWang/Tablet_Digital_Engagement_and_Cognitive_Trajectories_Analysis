...
df_T3_ALL_freq_summary <- df_T3_ALL |>
  ...
  summarise(
    med = median(absolute_freq_monthly, na.rm = TRUE),
    Q1 = quantile(absolute_freq_monthly, 0.25, na.rm = TRUE),
    Q3 = quantile(absolute_freq_monthly, 0.75, na.rm = TRUE),
    .groups = "drop"
  )

pc <- ggplot(
  df_T3_ALL,
  aes(x = Months_From_Earliest, y = absolute_freq_monthly, color = Class)
) +
  geom_pointrange(
    data = df_T3_ALL_freq_summary,
  ...
  labs(
    x = "Months",
    y = "Frequency",
    title = "C. Absolute monthly frequency (TDE)",
...
ggsave(
  filename = "~/Downloads/figure_2c_monthly_tde_frequency.jpg",
  plot = pc,
  dpi = 600,
  width = 12,
  height = 8,
